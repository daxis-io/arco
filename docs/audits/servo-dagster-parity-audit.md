# Servo/Arco vs Dagster Parity Audit

> **SUPERSEDED (2025-12-19):** This document is superseded by `docs/plans/2025-12-19-servo-arco-vs-dagster-parity-audit.md` and `docs/audits/servo-dagster-exception-planning.md`. It uses older assumptions and non-repo-addressable citations; do not use it for current parity claims.
...
Servo/Arco demonstrates strong architectural alignment with Dagster's core orchestration model while making deliberate serverless-first trade-offs. The hybrid Rust control plane + Python data plane architecture positions us well for performance and ecosystem compatibility.
...
| 3 | **No declarative automation/freshness policies** | Dagster auto-materializes based on freshness; Servo requires explicit triggers | M |
| 4 | **Limited run re-execution UX** | Dagster: re-run from failure in UI; Servo: API only, no subset re-execution | M |
...
| 7 | **No dbt integration** | Dagster has first-class dagster-dbt; Servo lacks this | M |
...
| Capability | Servo/Arco Advantage | Evidence |
|------------|---------------------|----------|
...
| **Unified catalog + orchestration** | Single metadata layer for catalog and execution | Arco Architecture Part 1 + Servo integration |
| **Canonical serialization** | Deterministic hashing across languages | ADR-010, Part 2 §15 |

---

## 2. Glossary: Dagster Term → Servo/Arco Term

| Dagster Term | Servo/Arco Equivalent | Notes |
|--------------|----------------------|-------|
| Asset | Asset | Direct equivalent |
| Op | Task | Servo uses asset-centric model; ops are tasks within runs |
...
| Partition | Partition (multi-dimensional) | More flexible in Servo |
...
| Dagster Capability | Good in Dagster | Servo/Arco Equivalent | Status | Evidence | Gap Description | Recommended Approach | Complexity | Risk if Missing |
|--------------------|-----------------|----------------------|--------|----------|-----------------|---------------------|------------|-----------------|
...
| Local dev execution | Run locally for testing | servo dev command | **Yes** | Part 1 §7.2-7.3 | - | - | - | - |
...
| Prometheus metrics | Metrics for monitoring | servo_* metrics defined | **Yes** | Part 2 §9.1 | - | - | - | - |
...
| CLI workflows | Command-line tooling | servo CLI commands | **Yes** | Part 1 §7.2 | - | - | - | - |
| Local dev server | servo dev | **Yes** | Part 1 §7.3 | - | - | - | - |
| Manifest validation | Validate before deploy | servo deploy --dry-run | **Yes** | Part 1 §2.1 | - | - | - | - |
...
| dbt integration | dagster-dbt | Not designed | **No** | Not in docs | No dbt integration | Build servo-dbt package | M | High - dbt is ubiquitous |
...
- **Proposed solution:** Build servo-dbt package with asset generation from dbt manifest
- **Alternatives:**
...
| **Rationale (business)** | Daxis has its own UI; Servo is infrastructure. MVP needs CLI + API. |
...
| **Trigger to revisit** | >5 customers requesting standalone Servo deployment |
...
|----------|------|---------------------|
| Servo Orchestration Design (Part 1) | docs/plans/2025-01-12-arco-orchestration-design.md | §1-9 (all) |
| Servo Orchestration Design (Part 2) | docs/plans/2025-01-12-arco-orchestration-design-part2.md | §1-22 (all) |
...
| ADR-017 Cloud Tasks | docs/adr/adr-017-cloud-tasks-dispatcher.md | Task dispatch |
