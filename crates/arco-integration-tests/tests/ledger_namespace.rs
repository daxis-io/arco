...
async fn flow_ledger_namespace_does_not_collide_with_catalog_compaction() {
    ...
    // Write one catalog materialization record under `ledger/execution/...`.
    ...
    // Write one orchestration event under `ledger/flow/execution/...`.
    let run_id = RunId::generate();
    let flow_event = EventBuilder::run_started("acme", "production", run_id, "plan-1");
    LedgerWriter::new(storage.clone(), "execution")
        .append(flow_event)
        .await
