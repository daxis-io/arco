...
pub async fn consolidate_pending(
    ...
    for _ in 0..4 {
        let Some(plan) = Box::pin(worker.prepare_at(Utc::now())).await? else {
            return Ok(None);
        };
        let id = plan.job_id().clone();
        let mut progress = Box::pin(worker.start_at(&plan, Utc::now())).await?;
        let mut invalidated = false;
        for _ in 0..512 {
            let attempt = if progress.status == MaintenanceStatus::Active {
                worker.advance_at(&id, Utc::now()).await.map(|next| {
                    progress = next;
                    None
                })
            } else {
                worker.publish_at(&id, Utc::now()).await
            };
            match attempt {
                ...
                Err(CatalogError::PreconditionFailed { .. }) => {
                    ...
                    worker.abandon_at(&id, Utc::now()).await?;
                    invalidated = true;
                    break;
                }
