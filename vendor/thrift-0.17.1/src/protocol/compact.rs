    ...
    T: TReadTransport,
{
    ...
    fn read_list_set_begin(&mut self) -> crate::Result<(TType, i32)> {
        ...
        if possible_element_count != 15 {
            ...
            element_count = self.read_varint32()? as i32;
        }
    ...
    fn read_varint32(&mut self) -> crate::Result<u32> {
        ...
        Err(crate::new_protocol_error(
            crate::ProtocolErrorKind::InvalidData,
...
mod tests {
    ...
    #[test]
    fn must_reject_overlong_varint_i32() {
        let (mut i_prot, _) = test_objects();
        i_prot.transport.set_readable_bytes(&[0x80u8; 6]);

        let res = i_prot.read_i32();

        assert!(res.is_err());
        match res {
            Err(crate::Error::Protocol(e)) => {
                assert_eq!(e.kind, crate::ProtocolErrorKind::InvalidData);
            }
    ...
    #[test]
    fn must_accept_valid_5_byte_varint_i32() {
        let (mut i_prot, _) = test_objects();
        i_prot
            .transport
            .set_readable_bytes(&[0x80u8, 0x80, 0x80, 0x80, 0x0F]);

        assert!(i_prot.read_i32().is_ok());
    }

    #[test]
    fn must_reject_overlong_varint_i64() {
        let (mut i_prot, _) = test_objects();
        i_prot.transport.set_readable_bytes(&[0x80u8; 11]);
