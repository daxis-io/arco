...
def run_init(*, name: str, template: str) -> None:  # noqa: ARG001
    ...
    (project_dir / "assets" / "example.py").write_text(
        dedent('''
    ...
    (project_dir / ".env.example").write_text(
        dedent("""
        # Arco Flow configuration
        ARCO_FLOW_TENANT_ID=your-tenant-id
        ARCO_FLOW_WORKSPACE_ID=development
        ARCO_FLOW_API_KEY=your-api-key

        # Optional
        # ARCO_FLOW_API_URL=https://api.arco.dev
    """).strip()
        + "\n"
    )
    ...
    console.print("  arco-flow validate")
    console.print("  arco-flow deploy --dry-run")
