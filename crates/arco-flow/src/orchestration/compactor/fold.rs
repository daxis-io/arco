...
impl FoldState {
    ...
    /// Server-minted reference time for an event, from its event-id ULID.
    ///
    /// Event ids are ULIDs generated by the process that appends the event, so
    /// their embedded millisecond is a server clock reading that no worker can
    /// influence. Anything scheduled from event data — most importantly the
    /// retry deadline — is anchored here rather than on `event.timestamp`,
    /// which carries producer-supplied metadata.
    ///
    /// Returns `None` for ids that are not ULIDs (deterministic test ids,
    /// legacy events) or whose millisecond falls outside the representable
    /// range; callers fall back to the event timestamp, which is what those
    /// events have always used. No wall clock is consulted, so replay stays
    /// byte-stable.
    fn server_reference_time(event_id: &str) -> Option<DateTime<Utc>> {
        let ulid = Ulid::from_string(event_id).ok()?;
        let millis = i64::try_from(ulid.timestamp_ms()).ok()?;
        DateTime::<Utc>::from_timestamp_millis(millis)
    }

    /// Computes the deadline before which a `RetryWait` attempt must not be
    /// redispatched.
    ///
    /// Anchored on server-minted event-id time and computed with checked
    /// arithmetic. An overflow yields `None`, which the anti-entropy sweeper
    /// reads as "no deadline recorded" and bootstraps immediately: a retry that
    /// happens too early is recoverable, a retry that never happens is not.
    fn retry_deadline(
        event_id: &str,
        timestamp: DateTime<Utc>,
        attempt: u32,
    ) -> Option<DateTime<Utc>> {
        let anchor = Self::server_reference_time(event_id).unwrap_or(timestamp);
        anchor.checked_add_signed(Self::retry_backoff(attempt))
    }
            ...
            match outcome {
                ...
                TaskOutcome::Failed | TaskOutcome::Skipped | TaskOutcome::Cancelled => {
                    ...
                    if new_state == TaskState::RetryWait {
                        ...
                        // once `retry_not_before` has passed. The anchor is
                        // server-minted event-id time, never the worker's
                        // reported completion clock, and the addition is
                        // checked so a pathological id cannot panic or wrap.
                        task.retry_not_before =
                            Self::retry_deadline(event_id, timestamp, task.attempt);
                    } else {
                        task.retry_not_before = None;
                    }
                }
    ...
    fn extract_execution_metadata(
        ...
        if let Some(output) = output {
            if metadata.materialization_id.is_none() {
                metadata
                    .materialization_id
                    .clone_from(&output.materialization_id);
            }

            metadata.delta_table.clone_from(&output.delta_table);
            metadata.delta_version = output.delta_version;
            metadata.delta_partition.clone_from(&output.delta_partition);
        }
...
mod tests {
    ...
    fn test_failed_attempt_enters_retry_wait_with_scheduled_deadline() {
        ...
        let server_anchor = FoldState::server_reference_time(&failure.event_id)
            .expect("a runtime event id is a server-minted ULID");
        ...
        assert_eq!(
            task.retry_not_before,
            Some(server_anchor + FoldState::retry_backoff(1)),
            "fold must schedule the first retry deadline deterministically \
             from server-minted event-id time"
        );
    }

    /// H3: `retry_not_before` must be anchored on server-minted event-id time,
    /// never on the worker-reported completion clock. A worker reporting a
    /// completion decades away in either direction must still have its attempt
    /// retried inside the configured backoff.
    #[test]
    fn retry_deadline_ignores_extreme_worker_reported_completion_times() {
        for skew_days in [-3650_i64, 3650_i64] {
            ...
            let mut failure = task_finished_event_with_asset(
                "run1",
                "extract",
                1,
                &attempt_1_id,
            ...
            failure.timestamp = Utc::now() + chrono::Duration::days(skew_days);
            let server_anchor = FoldState::server_reference_time(&failure.event_id)
                .expect("a runtime event id is a server-minted ULID");
            state.fold_event(&failure);
            ...
            let deadline = task
                .retry_not_before
                .expect("a retry deadline must be scheduled");
            assert_eq!(
                deadline,
                server_anchor + FoldState::retry_backoff(1),
                "a worker clock {skew_days} days off must not move the retry deadline"
            );
            assert!(
                deadline <= Utc::now() + FoldState::retry_backoff(1),
                "the retry must fall inside the configured backoff, not decades away"
            );
        }
    }

    /// H3: extreme event-id timestamps must not panic or wrap the deadline.
    /// An unrepresentable deadline is recorded as `None`, which the sweeper
    /// bootstraps immediately rather than deferring the retry forever.
    #[test]
    fn retry_deadline_uses_checked_arithmetic_at_the_representable_maximum() {
        // The largest millisecond a ULID can encode (48 bits).
        let max_ulid = Ulid::from_parts((1_u64 << 48) - 1, u128::MAX).to_string();
        let reference = FoldState::server_reference_time(&max_ulid)
            .expect("the maximum ULID millisecond is representable");
        assert!(
            reference > Utc::now(),
            "the maximum ULID time is far future"
        );
        assert!(
            FoldState::retry_deadline(&max_ulid, Utc::now(), 1).is_some(),
            "adding the backoff to the maximum ULID time must not panic"
        );

        // A non-ULID id falls back to the event timestamp; at the maximum
        // representable instant the addition overflows and must yield `None`
        // rather than wrapping into the past or panicking.
        let deadline = FoldState::retry_deadline("evt_not_a_ulid", DateTime::<Utc>::MAX_UTC, 1);
        assert!(
            deadline.is_none(),
            "an overflowing retry deadline must be recorded as absent, not wrapped"
        );
    }
