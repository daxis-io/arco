...
fn worker_dispatch_envelope_writes_canonical_camel_case_json() {
    ...
    assert_eq!(value["heartbeatTimeoutSec"], 300);
...
#[test]
fn worker_dispatch_envelope_accepts_legacy_json_without_partition_scope() {
    let legacy = json!({
        "tenant_id": "tenant-a",
        "workspace_id": "workspace-b",
        "run_id": "run-123",
...
#[test]
fn worker_dispatch_envelope_omits_partition_scope_when_absent() {
    let mut envelope = sample_envelope();
    envelope.partition_key = None;
    envelope.heartbeat_timeout_sec = None;

    let value = serde_json::to_value(&envelope).expect("serialize envelope");

    assert!(value.get("partitionKey").is_none());
    assert!(value.get("heartbeatTimeoutSec").is_none());
}
