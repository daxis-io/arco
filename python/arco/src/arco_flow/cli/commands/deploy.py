...
if TYPE_CHECKING:
    from arco_flow._internal.registry import RegisteredAssetProtocol
...
def run_deploy(
    *,
    ...
    if dry_run:
        ...
        console.print(
            f"[green]✓[/green] Deployed {response.get('assetCount')} assets to {workspace_id}"
        )
...
def _print_summary(manifest: object, assets: Sequence[RegisteredAssetProtocol]) -> None:
    ...
    """
    from arco_flow.manifest.model import AssetManifest  # noqa: PLC0415
