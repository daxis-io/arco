# Servo/Arco vs Dagster — Exception Planning Document

**Date:** 2025-12-19 (v2 — revised per architecture feedback)
**Status:** Draft for stakeholder approval
**Related Audit:** [2025-12-19-servo-arco-vs-dagster-parity-audit.md](../plans/2025-12-19-servo-arco-vs-dagster-parity-audit.md)

---

## Purpose

This document captures **deliberate parity exceptions** — Dagster features that Servo/Arco will **not match** (either permanently or for a defined deferral period). Each exception requires explicit stakeholder sign-off before the platform can claim "production ready."
...
| **Rationale (Business)** | Servo is intentionally asset-first to reduce cognitive surface and focus product messaging. Supporting two parallel models fragments documentation, examples, and community. |
...
| Log viewer | **Deferred** | `servo logs` CLI in M1 |
...
| **Migration Path** | M1: CLI-only (`servo status --watch`, `servo logs`)<br>M2: Minimal viable UI (run list, task detail, logs)<br>M3: Full partition matrix + launchpad |
| **Workaround** | `servo status --watch` for live updates; Grafana dashboards for metrics |
...
| **User Impact** | Cannot track freshness of upstream data not materialized by Servo in M1/M2. |
...
| **Evidence** | `python/arco/src/servo/manifest/builder.py` — single manifest per deploy |
...
| Field | Value |
|---|---|
| **Why out of scope** | Dagster Cloud's managed control plane is a product offering, not an OSS feature. Servo/Arco IS the managed offering — we are the equivalent of Dagster Cloud, not Dagster OSS. |
