...
class DispatchWorker:
    ...
    def __init__(
        ...
        *,
        ...
        self._task_token = (
            config.task_token.get_secret_value()
            or config.api_key.get_secret_value()
            or "debug"
        )
        self._client = ServoApiClient(config)
    ...
    def _execute_asset(self, payload: DispatchPayload) -> object:
        ...
        if len(signature.parameters) > 1:
            msg = (
                "assets with dependencies are not supported by the minimal worker"
            )
            raise RuntimeError(msg)
...
class DispatchHTTPServer(ThreadingMixIn, HTTPServer):
    ...
    def __init__(self, server_address: tuple[str, int], handler: type[BaseHTTPRequestHandler], worker: DispatchWorker) -> None:  # noqa: D401
        self.worker = worker
...
def run_worker(
    *,
    ...
    if not config.tenant_id:
        err_console.print(
            "[red]✗[/red] Tenant ID not configured. Set SERVO_TENANT_ID."
        )
        raise SystemExit(1)
    if not config.workspace_id:
        err_console.print(
            "[red]✗[/red] Workspace ID not configured. Set SERVO_WORKSPACE_ID."
        )
        raise SystemExit(1)
    if not config.debug and not (
        ...
        err_console.print(
            "[red]✗[/red] API key or task token required. "
            "Set SERVO_API_KEY or SERVO_TASK_TOKEN."
        )
        raise SystemExit(1)
