...
use crate::paths::{
    orchestration_base_snapshot_dir, orchestration_l0_dir, orchestration_manifest_pointer_path,
};
...
/// Historical current-head legacy-manifest issue variants retained for report compatibility.
...
mod tests {
    ...
    async fn check_reports_only_orphaned_orchestration_artifacts() -> Result<()> {
        ...
        assert_eq!(report.missing_current_head_legacy_manifest_path, None);
        assert_eq!(report.current_head_legacy_manifest_issue, None);
