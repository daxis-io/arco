...
mod tests {
    ...
    const UC_PREVIEW_OPERATIONS: &[(&str, &str)] = &[
    ...
    async fn test_unity_catalog_requires_tenant_workspace_headers() -> Result<()> {
        ...
        for &(method, uri) in UC_PREVIEW_OPERATIONS {
    ...
    async fn test_unity_catalog_preview_routes_are_mounted() -> Result<()> {
        ...
        for &(method, uri) in UC_PREVIEW_OPERATIONS {
            ...
            assert_eq!(
                ...
                "expected preview Not Implemented for {method} {uri}"
            );
            ...
            assert_eq!(
                error.get("error_code").and_then(|value| value.as_str()),
                Some("NOT_SUPPORTED"),
                "expected NOT_SUPPORTED for {method} {uri}"
            );
        }
        Ok(())
    }
