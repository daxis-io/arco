...
impl GarbageCollector {
    ...
    async fn delete_prefix(&self, prefix: &str) -> Result<(u64, u64)> {
        ...
        for meta in entries {
            ...
            if self.snapshot_path_is_currently_referenced(&path).await? {
                tracing::warn!(
                    path = %path,
...
mod tests {
    ...
    #[tokio::test]
    async fn delete_prefix_skips_snapshot_files_when_pointer_targets_version_at_delete_time() {
        let backend = Arc::new(MemoryBackend::new());
        let storage = ScopedStorage::new(backend, "acme", "prod").expect("storage");
        let tier1_writer = crate::Tier1Writer::new(storage.clone());
        tier1_writer.initialize().await.expect("init");
        ...
        let legacy_manifest = catalog_manifest_for_version(2, 2, "current.parquet", 2);
        storage
            .put_raw(
                &root.catalog_manifest_path,
                Bytes::from(serde_json::to_vec(&legacy_manifest).expect("serialize legacy")),
        ...
        let manifest_v2_path =
            CatalogPaths::domain_manifest_snapshot(CatalogDomain::Catalog, "00000000000000000002");
        let manifest_v2 = catalog_manifest_for_version(2, 2, "current.parquet", 2);
        storage
            .put_raw(
                &manifest_v2_path,
                Bytes::from(serde_json::to_vec(&manifest_v2).expect("serialize v2")),
                WritePrecondition::DoesNotExist,
            )
            .await
            .expect("write v2");

        let pointer_v2 = DomainManifestPointer {
            manifest_id: "00000000000000000002".to_string(),
        ...
        storage
            .put_raw(
                &CatalogPaths::domain_manifest_pointer(CatalogDomain::Catalog),
                Bytes::from(serde_json::to_vec(&pointer_v2).expect("serialize pointer")),
                WritePrecondition::DoesNotExist,
            )
            .await
            .expect("write pointer");

        let old_file = CatalogPaths::snapshot_file(CatalogDomain::Catalog, 1, "old.parquet");
        storage
            .put_raw(
                &old_file,
                Bytes::from_static(b"old"),
        ...
        let manifest_v3_path =
            CatalogPaths::domain_manifest_snapshot(CatalogDomain::Catalog, "00000000000000000003");
        let manifest_v3 = catalog_manifest_for_version(1, 3, "old.parquet", 3);
        storage
            .put_raw(
                &manifest_v3_path,
                Bytes::from(serde_json::to_vec(&manifest_v3).expect("serialize v3")),
                WritePrecondition::DoesNotExist,
            )
            .await
            .expect("write v3");
        let pointer_v3 = DomainManifestPointer {
        ...
        storage
            .put_raw(
                &CatalogPaths::domain_manifest_pointer(CatalogDomain::Catalog),
                Bytes::from(serde_json::to_vec(&pointer_v3).expect("serialize pointer v3")),
                WritePrecondition::None,
            )
            .await
            .expect("update pointer");

        let collector = GarbageCollector::new(storage.clone(), RetentionPolicy::default());
        let (count, _bytes) = collector
            .delete_prefix(&CatalogPaths::snapshot_dir(CatalogDomain::Catalog, 1))
            .await
            .expect("delete prefix");
        assert_eq!(count, 0, "must not delete newly referenced snapshot files");
        assert!(
            storage
                .head_raw(&old_file)
                .await
                .expect("head old")
                .is_some(),
            "old snapshot file must remain after guard skip"
        );
    }
}
